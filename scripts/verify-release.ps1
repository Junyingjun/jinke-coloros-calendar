...
$identity = Get-Content (Join-Path $projectRoot "project-identity.json") -Raw | ConvertFrom-Json
$version = [string]$identity.version
$expectedTag = "v$version"
$expectedApk = "jinke-coloros-v$version.apk"
...
if ($release.tag_name -ne $expectedTag) { throw "GitHub 最新版本不是 $expectedTag。" }
$apk = $release.assets | Where-Object { $_.name -eq $expectedApk } | Select-Object -First 1
