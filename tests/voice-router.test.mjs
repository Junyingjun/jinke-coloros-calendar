...
assert.match(appSource, /firstDailyOccurrenceDateKey[\s\S]*setSelectedDateKey\(visibleDateKey\)/, "a newly created daily task must navigate to its first visible occurrence");
assert.match(appSource, /writeStoredJson\("jinke-daily-tasks", next\)/, "new daily tasks must be persisted immediately");
assert.match(appSource, /saveAppState/, "task persistence must have a native fallback on ColorOS WebView builds");
...
assert.match(primitivesSource, /delta >= 0 \? delta : -Math\.min\(30, Math\.abs\(delta\) \* 0\.2\)/, "sheet handles must allow a damped upward pull before returning to rest");
...
assert.match(appSource, /const \[secondaryStack, setSecondaryStack\][\s\S]*pushSecondary[\s\S]*secondaryStack\.map/, "menu child and tertiary screens must use a layered bottom-sheet stack");
...
assert.match(appSource, /const handleNativeBack[\s\S]*if \(secondary\)[\s\S]*if \(overlay\)[\s\S]*viewMode === "month"[\s\S]*activeTab === "critical"[\s\S]*window\.JINKE_NATIVE_BACK/, "native back must unwind the in-app navigation hierarchy from the topmost sheet");
...
assert.match(screensSource, /function sourceUrlForOnThisDayItem[\s\S]*!\/\^\\d\{1,4\}\\s\*年\$\/[\s\S]*content_urls\?\.desktop\?\.page/, "archive links must prefer the subject page over a generic year page");
assert.match(screensSource, /kind: kind === "诞辰" && isLivingBirthItem\(item\) \? "生日" : kind/, "living people must be labelled 生日 while deceased people retain 诞辰");
...
assert.match(stylesSource, /\.phone-shell-expanded \.critical-reminder-screen \.reminder-default-card > \.reminder-rule-grid\s*\{[^}]*repeat\(2/, "expanded devices may restore the two-column reminder rule layout inside the reminder card");
...
assert.match(indexSource, /app-bundle\.js\?v=1\.1\.3/, "release HTML must cache-bust the current reminder-settings bundle");
assert.match(serviceWorkerSource, /CACHE_NAME = "jinke-v1\.1\.3"/, "the service worker cache must advance with the APK version");
