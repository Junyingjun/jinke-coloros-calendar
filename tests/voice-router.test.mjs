...
assert.match(indexSource, /app-bundle\.js\?v=1\.1\.1/, "release HTML must cache-bust the current reminder-settings bundle");
assert.match(serviceWorkerSource, /CACHE_NAME = "jinke-v1\.1\.1"/, "the service worker cache must advance with the APK version");
