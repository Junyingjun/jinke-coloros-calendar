...
已完成的关键事项在编辑器中额外显示自定义“完成日期”控件，完成时刻不作为归档字段公开；未完成事项不显示该字段。保存时必须原子更新 `completedDateKey`、`completedAt`、历史幂等键与日期标记，并将每日检视切换到新的完成日期，避免误取消后重新完成只能落在当天。未完成关键事项的截止日期由系统今天兜底；已完成事项可选择历史截止日期，完整 ISO 日期用于保留真实年份。
